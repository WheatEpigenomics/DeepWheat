...
def load_true_labels(labels_path):
    try:
        true_labels = pd.read_csv(labels_path, delimiter="\t").values.flatten()
        logging.info(f"Loaded true labels from {labels_path}, shape: {true_labels.shape}")
        return true_labels
    except Exception as e:
...
def main():
    try:
        ...
        try:
            mse = tf.keras.metrics.mean_squared_error(true_labels, predictions.flatten()).numpy()
            mae = tf.keras.metrics.mean_absolute_error(true_labels, predictions.flatten()).numpy()
            mape = tf.keras.metrics.mean_absolute_percentage_error(true_labels, predictions.flatten()).numpy()
            rmse = np.sqrt(mse)
            r2 = r2_score(true_labels, predictions.flatten())
            pearson_corr = scipy.stats.pearsonr(true_labels, predictions.flatten())

            logging.info(f"Prediction MSE: {mse}")
            logging.info(f"Prediction MAE: {mae}")
            logging.info(f"Prediction MAPE: {mape}")
            logging.info(f"Prediction RMSE: {rmse}")
            logging.info(f"Prediction R² Score: {r2}")
            logging.info(f"Prediction Pearson Correlation: {pearson_corr}")
        except Exception as e:
        ...
        try:
            ...
            logging.info("Predictions and evaluation metrics saved to need_predict_results.csv")
        except Exception as e:
        ...
        try:
            ...
            logging.error(f"Error plotting scatter plot: {e}")
            raise
